...
- **`report_render.py` owns all HTML rendering**, including the SELF-vs-HR audience-conditional branches; `build_html.py` is only argument parsing and file loading. When adding a section, decide its audience gate first (see the "V4 audience-conditional rendering" table in SKILL.md). HR output must never leak session IDs or non-allowlisted project names.
